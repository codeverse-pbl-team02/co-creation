        ...
        <div className="flex flex-col bg-background animate-in fade-in slide-in-from-bottom duration-300 min-h-full w-full">
          ...
          <div className="shrink-0 px-6 py-4 flex items-center justify-between border-b border-border/40 bg-card">
            <h2 className="text-base font-extrabold text-foreground flex items-center gap-2">
              <span className="text-lg">{certifyingType === "plogging" ? "♻️" : certifyingType === "barrierfree" ? "♿" : "🐕"}</span> {certifyingType === "plogging" ? "플로깅 인증하기" : certifyingType === "barrierfree" ? "배리어프리 인증하기" : "반려견 산책 인증하기"}
            </h2>
          ...
          <div className="p-5 space-y-5 pb-8">
            <div>
              ...
              {certifyingType === "pet" && (
                <p className="text-[10px] text-muted-foreground mt-0.5">📅 진행 기간: 상시 진행 (매일 참여 가능)</p>
              )}
            </div>
            ...
            {!showSuccess ? (
              <>
                    ...
                    <div className="flex flex-col gap-2">
                      <label className="flex flex-col items-center justify-center border-2 border-dashed border-primary/40 hover:border-primary/70 rounded-2xl p-6 bg-primary/5 hover:bg-primary/10 cursor-pointer transition-all text-center">
                        ...
                        <span className="text-[10px] text-muted-foreground mt-1">
                          {certifyingType === "plogging" ? "수거한 쓰레기 사진을 올려주세요" : certifyingType === "barrierfree" ? "활동 인증 사진 또는 GPS 캡처를 올려주세요" : "반려견과 함께한 산책 사진을 올려주세요"}
                        </span>
                        <input
                          type="file"
                          accept="image/*"
                ...
                {certifyingType === "plogging" ? (
                  ...
                  <>
                    {/* Run Distance Slider for Pet */}
                    <div className="space-y-3">
                      <div className="flex justify-between items-center">
                        <span className="text-xs font-bold text-foreground">인증할 산책 거리</span>
                        <span className="text-xs font-mono font-bold text-[#60AEDD] bg-[#60AEDD]/10 px-2 py-0.5 rounded-full">
                          {runDistance.toFixed(1)} km
                        </span>
                      </div>
                      <input
                        ...
                        value={runDistance}
                        onChange={(e) => setRunDistance(parseFloat(e.target.value))}
                        className="w-full h-1.5 bg-muted rounded-lg appearance-none cursor-pointer accent-[#60AEDD]"
                      />
                      <div className="flex justify-between text-[10px] text-muted-foreground">
                        <span>0.5km</span>
                        <span>2.7km</span>
                    ...
                    {/* Petiquette checklist */}
                    <div className="space-y-2">
                      <span className="text-xs font-bold text-foreground">준수한 펫티켓 항목 (중복 선택)</span>
                      <div className="grid grid-cols-2 gap-2 mb-2">
                        {["목줄/하네스 착용", "배변봉투 지참", "인식표 착용", "음용수 지참"].map((type) => {
                          const isSelected = selectedPetiquette.includes(type);
                          return (
                            <button
                              key={type}
                              onClick={() => togglePetiquette(type)}
                              className={`py-2 px-3 rounded-xl border text-xs font-semibold transition-all ${
                                isSelected
                                  ? "bg-[#60AEDD]/15 border-[#60AEDD] text-[#3B91C1]"
                                  : "bg-card border-border text-muted-foreground hover:bg-muted"
                              }`}
                            >
                              {isSelected ? "✓ " : ""} {type}
              ...
              <div className="space-y-4">
                <div className="flex flex-col items-center justify-center py-6 text-center space-y-4 animate-in zoom-in-95 duration-300">
                  <div className={`w-16 h-16 rounded-full flex items-center justify-center text-3xl ${
                    certifyingType === "plogging" ? "bg-[#6ACF98]/20 border border-[#6ACF98]/50" : certifyingType === "barrierfree" ? "bg-primary/20 border border-primary/50" : "bg-[#60AEDD]/20 border border-[#60AEDD]/50"
                  }`}>
                    {certifyingType === "plogging" ? "🌳" : certifyingType === "barrierfree" ? "🌈" : "🐕"}
                  </div>
                  <div className="space-y-1">
                    <h3 className="text-lg font-black text-foreground">인증 완료!</h3>
                    <p className={`text-xs font-bold ${certifyingType === "plogging" ? "text-[#4CB57D]" : certifyingType === "barrierfree" ? "text-primary" : "text-[#3B91C1]"}`}>
                      {certifyingType === "plogging" ? "초록빛 부산 만들기에 동참해주셨습니다." : certifyingType === "barrierfree" ? "장애 없는 배리어프리 세상을 위한 발걸음!" : "펫티켓을 준수하며 반려견과 산책했습니다!"}
                    </p>
                    <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-[#F7D66C]/20 border border-[#F7D66C]/40 rounded-full mt-2 text-xs font-bold text-[#B86F16] animate-bounce">
                      🏅 [{certifyingType === "plogging" ? "플로거" : certifyingType === "barrierfree" ? "낙동강 배지" : "반려견"}] 배지 획득!
                    </div>
                  </div>

                  <div className={`w-full rounded-2xl p-4 text-left border space-y-2 ${
                    certifyingType === "plogging" ? "bg-[#6ACF98]/10 border-[#6ACF98]/20" : certifyingType === "barrierfree" ? "bg-primary/5 border-primary/10" : "bg-[#60AEDD]/10 border-[#60AEDD]/20"
                  }`}>
                    <div className="flex justify-between text-xs">
                      <span className="text-muted-foreground">획득 보상</span>
                      <span className={`font-bold ${certifyingType === "plogging" ? "text-[#4CB57D]" : certifyingType === "barrierfree" ? "text-primary" : "text-[#3B91C1]"}`}>
                        {certifyingType === "plogging" ? "+100 그린 마일리지" : certifyingType === "barrierfree" ? "+150 그린 마일리지" : "+120 그린 마일리지"}
                      </span>
                    </div>
                    {certifyingType === "plogging" ? (
                      <>
                      ...
                      <>
                        <div className="flex justify-between text-xs">
                          <span className="text-muted-foreground">지구 구하기</span>
                          <span className="font-bold text-[#3B91C1]">-{(runDistance * 0.12).toFixed(2)}kg CO₂ 절감</span>
                        </div>
                        <div className="flex justify-between text-xs">
                          <span className="text-muted-foreground">산책 거리</span>
                          <span className="font-bold text-foreground">{runDistance.toFixed(1)}km</span>
                        </div>
                        <div className="flex justify-between text-xs">
                          <span className="text-muted-foreground">준수 항목</span>
                          <span className="font-bold text-foreground truncate max-w-[150px]">
                            {selectedPetiquette.length > 0 ? selectedPetiquette.join(", ") : "목줄 및 배변봉투"}
                          </span>
                        </div>
                      </>
                    )}
